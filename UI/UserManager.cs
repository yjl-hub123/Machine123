...
namespace Machine
{
    public partial class UserManager : Form
    {
        ...
        private void CreateUserManagerView()
        {
            ...
            this.dataGridViewUser.Columns.Add("userID", "设备码");
        ...
        private void dataGridViewUser_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if(null != dgv && dgv.CurrentRow.Cells[2].Value!= null)
                {
        ...
        private void Btn_Click_AddUser(object sender, EventArgs e)
        {
            ...
            {
                ShowMsgBox.ShowDialog("无法添加此用户，请检查用户信息", MessageType.MsgAlarm);
            }
        }
        ...
        private void UserModifyLog(string actStr)
        {
            UserFormula user = new UserFormula();
            MachineCtrl.GetInstance().dbRecord.GetCurUser(ref user);
            string head = "修改时间,用户,操作,修改前用户名,修改后用户名,修改前权限,修改后权限";
            string content = string.Format("{0},{1},{2},{3},{4},{5},{6}",
                DateTime.Now.ToLongTimeString(),
                user.userName,actStr,
                dataGridViewUser.CurrentRow.Cells[2].Value.ToString(),
                this.txtUserName.Text, 
                dataGridViewUser.CurrentRow.Cells[4].Value.ToString(),
                this.cmbUserLevel.SelectedIndex);
            MachineCtrl.GetInstance().WriteCSV(@"D:\\InterfaceOpetate\\UserChangeLog", DateTime.Now.ToString("yyyyMMdd") + "用户修改.CSV", head, content);
        }
    }
}
